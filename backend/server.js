// ======================================================
// KAMI — server.js
// Páginas: index · perfil · cuenta · register · login
// API:     auth · profile · library
// DB:      PostgreSQL (pool compartido via db.js)
// ======================================================
...
// ── Interaction Counter Cache (memoria + Redis opcional) ──
global.interactionCache = { likes: 0, reposts: 0, replies: 0 };
global.interactionDeltas = new Map(); // postId -> { likes, reposts, replies }
global.io = null;
...
} catch (e) { /* Redis no disponible */ }

// Cache helper que usa Redis si está disponible, sino memoria
global.cache = {
  async get(key) {
    if (redis) return redis.get(key);
    return null; // sin Redis, el cache se maneja en memoria via interactionDeltas
  },
...
// Cada 10s: consolidar contadores acumulados a PostgreSQL
setInterval(async () => {
  ...
  for (const [postId, delta] of entries) {
    ...
    } catch (e) { /* ignorar errores de consolidación */ }
  }
  }, 10000);

// Cada 5 minutos: sincronización completa de contadores desde feed_interactions
setInterval(async () => {
  try {
    await pool.query(`
...
// ======================================================
// GENERAR db.js SI NO EXISTE
// (todas las rutas hacen: require('../db'))
// ======================================================
...
pool.connect()
    .then(client => {
        ...
        client.release();
        // Sincronizar contadores desde feed_interactions (tras reinicio)
        pool.query(`
          UPDATE feed_posts fp SET
            like_count = (SELECT COUNT(*) FROM feed_interactions fi WHERE fi.post_id = fp.id AND fi.interaction_type = 'like'),
            repost_count = (SELECT COUNT(*) FROM feed_interactions fi WHERE fi.post_id = fp.id AND fi.interaction_type = 'repost'),
...
// IP Ban middleware con caché en memoria (evita consultas SQL en cada asset)
const ipBanCache = new Map();
const IP_BAN_TTL = 60_000; // 1 minuto

app.use(async (req, res, next) => {
  if (!req.path.startsWith('/api/')) return next();
  if (req.path === '/api/premium/webhook') return next();

  const clientIp = (req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.ip || req.connection?.remoteAddress || '0.0.0.0').replace(/^::ffff:/, '');
...
// Forzar no-cache en todas las respuestas HTML
...
// Cache buster automático: reemplaza ?v=XX por timestamp en cada HTML servido
(function() {
  ...
  express.response.sendFile = function(filePath, options, callback) {
    if (typeof filePath === 'string' && filePath.endsWith('.html') && this.req) {
      ...
      fs.readFile(filePath, 'utf8', function(err, data) {
        ...
        data = data.replace(/\?v=\d+/g, '?v=' + Date.now());
...
// Assets estáticos: CSS, JS cliente, imágenes, etc.
app.use(express.static(path.join(__dirname, 'public'), {
  ...
  setHeaders: function(res, filePath) {
    if (filePath.endsWith('.html')) {
      ...
      // Los CSS/JS se cachean 1 día - el HTML ya tiene ?v=timestamp para invalidar
...
// No cache for HTML pages served via routes
...
app.get('/inicio-mangoteca', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'inicio-mangoteca.html'));
});

// Mangoteca (redirige a inicio-mangoteca)
app.get('/mangoteca', (req, res) => {
    res.redirect('/inicio-mangoteca');
});

// NUEVO — Registro (botón "Registrarse" del nav de index.html)
...
app.get('/perfil', noCache, (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'perfil.html'));
});


// pd2 — Perfil  /profile/:username
app.get('/profile/:username', noCache, (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'perfil.html'));
});

// Perfil editor — /perfil-editor/:username
app.get('/perfil-editor/:username', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'perfil-editor.html'));
});

// pd3 — Mi Cuenta / Configuración
...
app.get('/generos', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'generos.html'));
});

// Buscar (Comunidad search results)
...
app.get('/add-manga', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'add-manga.html'));
});

// Menu-edit — Buscador de mangas existentes (reemplaza a editor.html)
app.get('/menu-edit', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'menu-edit.html'));
});

// Subir capítulo
app.get('/manga/:id/upload-chapter', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'upload-chapter.html'));
});

// Manga — Vista consumidor
app.get('/manga/:id/edit', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'manga-edit.html'));
});

app.get('/manga/:id', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'manga-view.html'));
});
...
app.get('/reader/:mangaId/:chapterNum', (req, res) => {
...
// Limpiar tokens revocados expirados cada hora
